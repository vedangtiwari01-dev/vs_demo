...
class RuleValidator:
    ...
    # Phase 5 Enhancement: REMOVED HARDCODED APPROVAL_HIERARCHY
    # Approval authority rules now extracted from SOP via enhanced LLM prompt
    # Evaluated by ConditionalRuleEvaluator (Phase 4) instead of hardcoded logic
    ...
    def _check_approval_rules(case_id: str, logs: List[Dict[str, Any]], rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Check if basic approval rules are followed.

        NOTE: Approval authority hierarchy validation (LTV, credit score, loan amount thresholds)
        is now handled by ConditionalRuleEvaluator using rules extracted from SOP.
        This method only performs basic sanity checks.
        """
        deviations = []
        ...
        # Basic validation: Check if approval steps exist
